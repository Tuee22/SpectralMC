...
async def test_gc_minimum_versions_enforced(
    ...
    # This is < keep_min_versions=3, so should raise error
    with pytest.raises(ValueError, match="minimum is 3"):
        await gc.collect(dry_run=True)
...
async def test_gc_dry_run_vs_actual(async_store: AsyncBlockchainModelStore) -> None:
    ...
    # Dry run
    report_dry = await gc.collect(dry_run=True)
...
async def test_run_gc_convenience_function(
    ...
    report = await run_gc(async_store, keep_versions=4, protect_tags=[2], dry_run=True)
...
async def test_gc_policy_validation(async_store: AsyncBlockchainModelStore) -> None:
    ...
    # Should raise error during collection
